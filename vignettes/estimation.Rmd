...
changepoint_detection(Y, L = 3, t = c(2, 100, seq(200, length(Y) - 1, by = 200)),
                      verbose = TRUE)
```
...
softmax <- function(x) exp(x - log_sum_exp(x))

plot(lp, xlab = 't', main = 'Log probability from joint posterior (Stan)', type = 'l')
plot(softmax(lp), xlab = 't', main = 'Stan changepoint algorithm output', type = 'l')
